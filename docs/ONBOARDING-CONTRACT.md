...
| Complete retried while the original is still running | The retry waits for the original, then returns its recorded result. If the original committed nothing, the retry is evaluated as a new completion with its own outcome. If the wait passes aicrew's bound, the retry is refused with the retryable `request_in_progress`; if the caller cancels it or its deadline passes first, the caller gets its own cancellation instead. Neither changes anything, and the client retries with the same key. |
